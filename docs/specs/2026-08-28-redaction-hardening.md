...
| H5 | `kali_pentest_server.py:557` (`_clip` via the guard's return) | cap violation | **Claim corrected.** Line 557 returns `text[:match.end()] + "[REDACTED]"`, so the result length is `match.end() + 10`, NOT `limit + 10`: overshoot needs the opener to end exactly at the cap. Issue #27's stated input `"A"*8167 + "eyJabcdefgh." + "B"*500` gives 8189, three UNDER the 8192 cap. Reproduced worst case is `"A"*8180 + ...` → 8202, +10. **Second correction, from a later round: "only the JWT opener can overshoot" was itself wrong.** Measured overshoot of the raw guard is JWT 10 and URL 5, and row `F2` records the URL case directly (`"A"*8171 + "https://svc:BBBB"` -> 8193 against an 8192 cap). The reasoning above was right about PEM, which uses `keep_opener=False` and cuts before the opener, and wrong about URL: the `continue` at line 553 fires only when `URL_PORT_TAIL` matches, which an unterminated userinfo does not reach. Compounds at line 1529, `_clip(body, MAX_EVIDENCE_CHARS - len(cut)) + cut`. | fixed |
...
prefixes, secret bodies and trailing text. It grew 512 -> 672 at `c033087`, which
added five bodies, and 672 -> 800 at `65b8b6b`, which added four whitespace-bearing
ones. The bodies themselves are listed in `SWEEP_BODIES`; every attempt to
characterise them in prose here has been wrong, three times running, so the count
and the revision are all this paragraph now claims: the 36-sample corpus is blind to the G1
...
product. Its replacement, "port-shaped and empty-username credentials", was wrong
too: it describes three of the five. Both are recorded rather than quietly
overwritten, because a correction that introduces a new false claim is the failure
this section is about, and it happened twice on this one paragraph.
...
| R8-W2 | Spec axis | wrong claim | The terminator coverage test's docstring claimed the strict set "was pinned by nothing at all". False: the wide set already contained `/?#` and the loop iterated it. After the derivation the strict set is a subset by construction, so the added loop can never fail while the wide loop passes. The loop stays — it is what fails if the sets are ever un-derived — and the docstring now says that instead. | fixed |
| R9-N3 | red team | **gate fails on the wrong thing** | `workflow_dispatch` on `main` fails 100% of the time: no PR base, merge-base equals HEAD, no `before`. One of three declared triggers permanently red. | fixed |
| R9-N4 | red team | wrong claim | The step's comment describes a "push build" measuring a whole feature branch. `push` is filtered to `branches: [main]`, so that build never runs; `pull_request` sets the base and skips the branch entirely. A comment describing a build that cannot happen. | fixed |
...
| R9-N1 | red team | unpinned invariant | Deleting `limit = max(limit, 0)` from `_clip` left the suite green: the negative half of the invariant this round wrote down was unmeasured. | fixed |
| R9-N2 | red team | dead branch | `max(0, limit - len("[REDACTED]"))` is redundant once the single return bounds the result. One fewer branch in a function that has broken three times. | fixed |
| R9-N5 | red team | gap | No `merge_group` trigger: under a merge queue neither the acceptance step nor the differential gate runs on the queued merge. Nothing is broken today and it matters only if a merge queue is enabled. **Ruled closed by the user**, along with the remaining cosmetic items (F5 naming, F6's dead `sweep` parameter, F8's six `"[REDACTED]"` literals; F4 is obsolete, the script it complained about is no longer bash). | declined |
| R9-N8 | red team | test gap | `tests/test_mutation_check.py` covered `verdict()` only; `main()` was unpinned, and R8-B2 is what that gap let through. The payload channel is now pinned by `PayloadChannelTests`. Restore-on-exit, cache clearing and the dirty-tree refusal remain unpinned: testing them needs a scratch git repo per case, and that was ruled too much machinery for the value. | fixed |
| R9-N9 | red team | guard gap | `git diff --quiet -- <path>` did not see STAGED changes, so `git add` bypassed the dirty-tree refusal. Pre-existing, carried from the bash version, not introduced by the rewrite. The script ships `git diff --quiet HEAD -- <path>` and refuses: a staged edit gives `refusing to run`, exit 2. | fixed |
| R10-B1 | red team + Standards | **gate passed vacuously** | Exit 2 was given to "nothing to measure", but 2 is not a value only this script produces: CPython exits 2 on a script it cannot open, and `argparse` exits 2 on any usage error. Observed with the differential DELETED: the step printed `can't open file`, then a notice asserting the base carried an identical file, then exited 0. The gate greened on a deleted gate path and stated a false reason. Nothing-to-measure moved to 3, which neither produces, and the contract is pinned by a test — nothing pinned it before, which is why the collision shipped. | confirmed |
| R10-B2 | red team | **drift, FIFTH recurrence** | Round 9's `getattr(self.server, name, "")` — added so the suite could replay against a base predating the constants — also fires on a RENAME, so the loop iterates an empty string and passes. Observed: rename the wide set, add `~` to it, pattern follows the new name, suite green with `~` pinned by nothing. The default cannot simply be dropped: `b3ca7e1` carries neither constant, so direct access errors and R8-P1 reopens. Absence is now an assertion FAILURE (which the mutation gate accepts as proof) rather than an error (which it rejects) or a silent skip. | confirmed |
| R10-V1 | Standards | drift | `CLAUDE.md` still described the differential as a two-state gate after it gained a third state, while `CLAUDE.md`'s own "Adding or changing a tool" section requires updating current docs. Anyone following it locally would read a pass as a failure. | confirmed |
| R10-V4 | Standards | fd leak | `run_suite` closes only `write_fd` in its `finally`, so `read_fd` leaks if `subprocess.run` raises. | confirmed |
| R10-J1 | Standards | dead code | A `pull_request_target` arm in the base-resolution `case`, for an event this repo deliberately does not enable, two lines under a comment enumerating "all three this workflow declares". Removed, not added as a trigger: `pull_request_target` runs untrusted PR code against the base repo's secrets. | fixed |
| R10-J2 | Standards | drift risk | The exit-code contract was spelled in four hand-kept places — the script, the workflow, this spec, and `CLAUDE.md`, which contradicted the other three — with no constant and no test. Same shape as the terminator drift this branch fixed by derivation, and that lesson was not carried across. R10-B1 is what it cost. | confirmed |
| R10-N1 | red team | unpinned branch | Deleting `_clip`'s whole `if len(clipped) > limit:` re-guard leaves the suite green: the cap invariant is satisfied by the single final slice alone, so the cap test cannot see the branch go. Not dead — it changes output in thousands of probes — but nothing asserts its presence or its effect. Fourth consecutive round touching this function. | confirmed |
| R10-N2 | red team | hang, not failure | The payload read blocks without bound if any process inherits the write fd and outlives the child (observed with a forking test: the read never returns). And deleting `os.close(write_fd)` makes the suite HANG rather than fail, so that line is pinned only by a wall clock, in a CI job declaring no `timeout-minutes`. A check whose failure mode is a hang reads as infrastructure trouble, not a defect. | confirmed |
| R10-N3 | red team | asymmetric verdict | `scripts/mutation-check HEAD` returns the loudest possible accusation — "your assertions pin nothing" — for a run where nothing was mutated and nothing could be caught. Its sibling gate got a dedicated code for that exact byte-identical case in the same commit. | confirmed |
| R10-F3 | Spec axis | wrong claim | The R8-W1 replacement was itself wrong: "port-shaped and empty-username credentials" describes three of the five bodies. Third consecutive false statement in one paragraph, so the paragraph stopped characterising them. | fixed |
| R10-H5n | Spec axis | wrong claim, PRE-EXISTING | Row `H5` reads "Only the JWT opener can overshoot", which row `F2` already contradicts. Measured overshoot of the raw guard: JWT 10, URL 5. Corrected in place on the user's ruling, as a SECOND correction appended to the row rather than a rewrite of it, so the row now carries both what was claimed and what measurement showed. | fixed |
