...
| R10-H5n | Spec axis | wrong claim, PRE-EXISTING | Row `H5` reads "Only the JWT opener can overshoot", which row `F2` already contradicts. Measured overshoot of the raw guard: JWT 10, URL 5. Outside this diff and left as a note rather than a rewrite, since editing a closed round's row would rewrite history this table exists to keep. | filed |
